...
- **[openclaw/openclaw](https://github.com/openclaw/openclaw)** &nbsp;<img src="https://img.shields.io/github/stars/openclaw/openclaw?style=social" alt="stars" valign="middle">
  - **P0 release blocker:** the managed Gateway installed but never started on every non-CJK Windows locale — its generated `.cmd` launcher was written in UTF-8 instead of the system's boot-time OEM code page ([PR #108967](https://github.com/openclaw/openclaw/pull/108967))
  - **Burned tokens on every turn of every long session:** staged compaction re-added the previous summary to a result that already carried it, so the summary grew a copy of itself each cycle. Fixed by keying the fallback on the *oldest* split — the one signal the consumer needs — with an end-to-end regression test ([PR #109828](https://github.com/openclaw/openclaw/pull/109828))
  - **Every file an agent created on Windows came back lowercased**, so the import it had just written broke for all teammates on Linux and in CI; Turkish names like `İstanbul.md` came back corrupted, not merely lowercased. The sandbox helper was returning a comparison key as a real path — fixed with a case-preserving normalizer, every boundary verdict proven unchanged ([PR #109823](https://github.com/openclaw/openclaw/pull/109823))
  - **Billed work recorded as free: $0.006 → $0 and 1680 → 0 tokens per truncated turn.** Every answer that hit the output cap ended `incomplete`, matched no terminal branch, and was persisted as a successful turn that spent nothing — so nothing downstream could retry or warn. Fixed by finalizing both terminal events through one canonical usage mapper shared with the package side ([PR #109904](https://github.com/openclaw/openclaw/pull/109904))
  - **Long code-fence replies were dropped outright** — 41 chunks over Discord's 2000-character limit, rejected with HTTP 400, and only 41 of 1290 body characters surviving. The chunker reopened each continuation with the full opening line while budgeting only the closing marker: 1534 chunks became 4, all sendable, body intact ([PR #110148](https://github.com/openclaw/openclaw/pull/110148))
